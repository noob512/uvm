 ...
 *
 * always_max intra-block + direction-aware cross-block (1-block)
 * + prefetch-informed eviction: eviction checks a shared ring buffer
 *   of recent XB prefetch targets, protecting blocks near those targets
 *   from premature eviction.
 *
 * Eviction: cycle_moe + prefetch target protection
 */
...
#define VA_BLOCK_MASK (~(VA_BLOCK_SIZE - 1))
#define DEFAULT_PREFETCH_LEN VA_BLOCK_SIZE

#define PREFETCH_RING_SLOTS 8

/* Configurable via rodata */
const volatile int protect_radius = 2; /* protect blocks within ±N of XB target */
...
/* ===== Cross-block prefetch work queue ===== */
...
/* ===== Prefetch target ring buffer (shared between prefetch and eviction) =====
 * key 0 = write head index
 * keys 1..PREFETCH_RING_SLOTS = VA targets (block-aligned)
 */
struct {
	__uint(type, BPF_MAP_TYPE_ARRAY);
	__uint(max_entries, 9); /* 1 + PREFETCH_RING_SLOTS */
	__type(key, u32);
	__type(value, u64);
} prefetch_ring SEC(".maps");
...
#define STAT_COOP_PROTECT 10
#define STAT_T1_PROTECT   11
#define NUM_STATS         12
...
{
	u64 *val = bpf_map_lookup_elem(&coop_stats, &key);
	if (val)
...
/* Helper: write target to prefetch ring */
static __always_inline void ring_push(u64 target_va)
{
	u32 head_key = 0;
	u64 *head = bpf_map_lookup_elem(&prefetch_ring, &head_key);
	if (!head)
		return;

	u64 h = *head;
	u32 slot = (u32)(h % PREFETCH_RING_SLOTS) + 1; /* slots 1..8 */
	bpf_map_update_elem(&prefetch_ring, &slot, &target_va, BPF_ANY);
	*head = h + 1;
}

/* ===== PREFETCH: always_max + direction-aware XB ===== */
             ...
             uvm_va_block_region_t *result_region)
{
	...
	/* 3) Record target in prefetch ring (for cooperative eviction) */
	ring_push(prefetch_target & VA_BLOCK_MASK);

	/* 4) Schedule cross-block prefetch via bpf_wq */
...
/* ===== EVICTION: cycle_moe + cooperative prefetch protection ===== */
             ...
             struct list_head *list)
{
	...
	if (va_block) {
		...
		u64 chunk_block = chunk_va & VA_BLOCK_MASK;

		/* Check against prefetch ring (bounded loop, 8 iterations) */
		for (int i = 1; i <= 8; i++) {
			u32 rk = (u32)i;
			u64 *target = bpf_map_lookup_elem(&prefetch_ring, &rk);
			if (!target || *target == 0)
				continue;

			u64 target_block = *target;
			/* Check if chunk is within ±protect_radius blocks of target */
			s64 diff = (s64)(chunk_block - target_block);
			if (diff < 0)
				diff = -diff;
	...
	/* 2) Standard cycle_moe: T1 frequency protection */
...
SEC(".struct_ops")
struct gpu_mem_ops uvm_ops_cooperative = {
