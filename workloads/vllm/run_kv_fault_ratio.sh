...
TRACE_LOG="/tmp/uvm_kv_fault_stats.log"
...
Options:
  ...
  --trace-log <path>         Trace capture file when --mode trace
...
start_trace_capture_if_needed() {
  ...
  sudo sh -c "cat '$TRACE_DIR/trace_pipe' | grep --line-buffered 'Replayable fault stats GPU' > '$TRACE_LOG'" &
  TRACE_PID=$!
  sleep 1
  kill -0 "$TRACE_PID" 2>/dev/null || die "Failed to start trace capture"
}
...
validate_inputs() {
  ...
  need_writable_param "uvm_perf_fault_kv_end"
    ...
    # dirname 获取文件路径的父目录，mkdir -p 确保多级目录都能被创建且不报错
...
print_recent_stats() {
  ...
  if [ "$MODE" = "dmesg" ]; then
    sudo dmesg -T | grep "Replayable fault stats GPU" | tail -n 20 || true
  else
    tail -n 20 "$TRACE_LOG" 2>/dev/null || true
  fi
}
...
main() {
  ...
  [ "$MODE" = "trace" ] && echo "Trace log: $TRACE_LOG"
